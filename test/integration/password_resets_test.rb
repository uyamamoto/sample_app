...
class PasswordUpdateTest < PasswordResetForm
  ...
  test "update with valid password and confirmation" do
    patch password_reset_path(@reset_user.reset_token),
          params: { email: @reset_user.email,
                    user: { password:              "foobaz111",
                            password_confirmation: "foobaz111" } }
  ...
  class ExpiredToken < PasswordResets
    ...
    def setup
      ...
      # ユーザーのパスワードの更新を試みる
      patch password_reset_path(@reset_user.reset_token), params: { email: @reset_user.email, user: { password: "foobar111", password_confirmation: "foobar111" } }
    end
  end
